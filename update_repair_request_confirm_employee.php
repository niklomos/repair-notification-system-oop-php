...
if (isset($_POST['update'])) {
    ...
    foreach ($_FILES['images']['tmp_name'] as $key => $tmp_name) {
        ...
        if (move_uploaded_file($tmp_name, $target_file)) {
            if (!$request->insertImageSuccess($req_id, $target_file)) {
                $upload_errors[] = "Failed to insert image data for $file_name.";
            }
        } else {
    ...
    if (empty($upload_errors)) {
        echo "<script>
            ...
            window.location.href = 'summary_employee_scheduled.php';
        </script>";
        exit;
    } else {
        echo "<script>
            alert('มีข้อผิดพลาด: " . implode(", ", $upload_errors) . "');
        </script>";
    }
}
...
<div class="container pb-3 ">
    <h1 class="mb-4">Confirm Repair</h1>
    ...
    <div class="mb-3">
        ...
        <button type="submit" name="update" class="btn btn-primary"><i class="fas fa-check me-1"></i>Completed Repair</button>
    </form>
</div>
